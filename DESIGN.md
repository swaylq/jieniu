...
暖色皮肤：**近黑暖墨 + 暖米画布 + 白卡**。**固定深色只有一处**：左侧工作台侧栏
（走 `--color-sb-*`，不随明暗模式翻转，因为它是工作台的台面而不是内容）。

内容区**全部**走语义变量（`surface / surface-2 / ink / muted / faint / line`），
`.dark` 覆盖同名变量后自动翻转。**不要再造第二处不翻转的深色块。**

> 2026-08-05 个股页顶部的结论卡曾照侧栏那套做成固定深色，理由是「深色＝这是结论」的
> 层级跳变；8-06 sway 否掉了：亮色模式下一整块黑压在暖米画布上太重，而且主题开关对它失效。
> **层级要靠结构撑（整宽 / 置顶 / 衬线大标题 / 外层 surface 配内层 surface-2），不要靠
> 「永远深色」撑。** 这条比那一次的例外更值钱，别再走回去。
...
**第一屏先回答「现在是什么状态」，再给原材料。** 个股页此前是反的——手机上要先滚过
行情、估值、一致预期、信号条（约 1600px）才看得到投资逻辑，也就是这个产品真正卖的东西。
现在顺序是：页头（名称 + 代码 + **实时价格**）→ 个股决策卡（结论 → 五个模型 → 我的条件完成度）
→ 投资逻辑 → 行情/估值/我的计划。

两条配套约束，改版式时别拆掉：

1. **结论卡上屏不能把价格挤走。** 这是个天天来看的工作台，价格必须留在首屏——所以有了
   页头那份紧凑行情（`entity/[id]/header-quote.tsx`，只读 `LiveQuote` 的 tRPC 缓存、
   自己不发请求）。
2. **一行一个模型，不要并排五张小卡。** 五个模型用竖排 `<details>`：390px 下每张小卡只剩
   70px 宽、档位词就顶满了；1500px 下展开一张会把整行撑起来、其余四张跟着变形。
   竖排两头都成立，且 `<details>` 是原生的——服务端组件不用为一个展开塞 JS 到客户端。
3. **五个模型的档位一律中性墨阶。** 它们每张卡都渲染五次，染色就成了底噪（见上面琥珀那条
   机械判定）。方向靠词区分——「改善 / 走弱 / 未变化 / 待确认」比颜色说得清楚，也不吃色盲的亏。
   这一屏唯一的琥珀留给「最大的不确定性」：它是唯一还需要用户去核的东西。
...
5. **取数只许一道波**：page.tsx 里是 `auth()` → 单个 `Promise.all`（15 个查询：14 解构 + 埋点）。原来是四道串行波
