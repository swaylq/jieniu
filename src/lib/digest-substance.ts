...
const CIRCULAR: RegExp[] = [
  ...
  // 「存储芯片概念震荡回升带动股价冲高」——上面几条要么盯「板块/行业」，要么要一个逗号；
  // 主语换成概念/题材/赛道、用「带动」直接连到股价时一条都拦不住（2026-08-03 复核线上 note 发现）。
  // 落点必须是**股价类**结果才判：「行业景气回升带动订单增长」是真解释，不能误伤。
  /(板块|行业|概念|题材|赛道)[^，。；,;]{0,10}(走强|走弱|回升|回落|反弹|回调|震荡|升温|降温|活跃|大涨|大跌|上涨|下跌|拉升|跳水)[^，。；,;]{0,8}(带动|推动|拉动|支撑|提振|拖累|压制|催化)[^，。；,;]{0,8}(股价|涨|跌|走高|走低|冲高|回落|上扬|下挫|表现|反弹)/,
];
