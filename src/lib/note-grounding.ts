...
export type GroundingItem = {
  ...
  subject: string;
  /**
   * 主体是一家公司还是一个板块。**必须分开说**：板块的归因里点名成分股是正常的
   * （「同板块的欣天科技实控人拟变更」），而按公司那套判据会被当成「张冠李戴」判掉——
   * 实测第一版就这么误杀了一条正确的板块归因。
   */
  kind?: "stock" | "sector";
...
3. **张冠李戴**——把别家公司的事说成这家公司的事；事实里这家公司只是被顺带提到（客户、供应商、
   同行、参股方），却被写成它自己发生的事。
   ※ 主体标着【板块】的那几条**不适用**这一条：板块归因里点名成分股是正常写法
   （「同板块的某某公司实控人拟变更」），只要那家公司的事在事实里就算 yes。
    ...
    .map((it, idx) => {
      const facts =
        it.facts.length > 0
          ...
          : "     （无）";
      const kind = it.kind === "sector" ? "【板块】" : "";
      return `${idx + 1}. 标的：${kind}${it.subject}\n   事实：\n${facts}\n   归因：${it.note}`;
    })
    .join("\n\n");
}
